...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
        ...
        if (supabase) {
          try {
            ...
            if (cached) {
              ...
              const staleness = checkCaptainStaleness(
                responseJson,
                bootstrap.elements,
                cached.generated_at,
                req.log,
      ...
      if (clientUserId && gameweek) {
        ...
        if (supabase) {
          try {
            ...
            if (transferRows?.[0]) {
              const outPlayers = extractTransferOutPlayers(
                transferRows[0].response_json as Record<string, unknown>,
              );
              if (outPlayers.length > 0) {
                transferContextPrompt = buildTransferContextPrompt(outPlayers);
                req.log.info(
                  { outPlayers },
                  "Transfer cross-check: excluding OUT players from captain picks",
                );
              }
            }
          } catch (err) {
            req.log.warn(
              { err },
              "Transfer cross-check lookup failed — generating without transfer context",
            );
          }
        }
      }
      ...
      if (activeChip === "3xc") {
        chipPrompt =
          "TRIPLE CAPTAIN ACTIVE: The manager has activated the Triple Captain chip. The captain will earn TRIPLE points this gameweek instead of double. This makes captain selection even more critical — prioritise safety and high-floor picks. Emphasise the magnified impact in your recommendations.";
      } else if (activeChip === "bboost") {
        chipPrompt =
          "BENCH BOOST ACTIVE: The manager has activated Bench Boost. All bench players will score points this gameweek. Consider this when making captain recommendations — bench players earning points means the captain differential is relatively less impactful, but still choose the highest-ceiling option.";
      } else if (activeChip === "wildcard") {
        chipPrompt =
          "WILDCARD ACTIVE: The manager has activated their Wildcard chip and can change their entire squad. Captain recommendations should reflect the squad they end up with — focus on the best available captain options from the full player pool, not just their current squad.";
      } else if (activeChip === "freehit") {
        chipPrompt =
          "FREE HIT ACTIVE: The manager has activated their Free Hit chip — they can pick any squad for this single gameweek. Captain recommendations should consider ALL available players since the squad will revert next week. Prioritise the absolute best captain pick regardless of current squad composition.";
      }
...
TONE GUARDRAILS: Never use metaphors involving weapons, violence, or aggression. The Critic is sarcastic and teasing, not hostile — think pub banter, not confrontation. All vibes should feel fun and safe.`,
      ]
        .filter(Boolean)
        .join("\n\n");
      ...
      if (recs && bootstrap && fixtures && gameweek) {
        ...
        if (filtered.length < 2) {
          req.log.warn(
            ...
            "Too few recs after hallucination check — retrying",
          );
          ...
          if (retryParsed) {
            const retryRecs = retryParsed.recommendations as
            ...
            if (retryRecs) {
              ...
              req.log.info(
                {
                  removed: retryResult.removedCount,
                  corrected: retryResult.correctedCount,
                  remaining: retryFiltered.length,
                },
      ...
      if (
        ...
        res
          .status(504)
          .json({
            error: "SuperScout is taking longer than usual — please try again in a moment.",
          });
      } else {

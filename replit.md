...
### Key Features
- **Captain Picker**: Provides AI-powered captain recommendations, displaying choices with confidence badges (Banker, Calculated Risk, Bold Punt). Free tier shows only the SuperScout Pick; Pro shows all 3-5 options. Handles edge cases: BGW/DGW banners, chip context (TC/BB/WC/FH), deadline-passed state with current captain display, season-not-started graceful fallback.

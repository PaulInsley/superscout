...
- **API Security**: Implements Helmet security headers, two-tier rate limiting for AI generation and data endpoints, Zod schema validation on all POST/PUT routes via `validateBody()` middleware, JWT-based `requireAuth` middleware (`authMiddleware.ts`) on all user-facing routes that validates tokens via Supabase and sets `req.verifiedUserId` + `req.userSupabase` (RLS-scoped), ownership checks (403) on all data access routes, admin rate limiting (50 req/15 min), timing-safe admin password comparison, COOKIE_SECRET enforcement, and service_role restricted to background/batch jobs only. Server-only batch routes (`update-all`, `send-batch`, `pre-generate`, `process-decisions`) use `PROCESS_DECISIONS_SECRET` header auth.
